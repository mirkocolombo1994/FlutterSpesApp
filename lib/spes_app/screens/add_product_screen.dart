...
class _AddProductScreenState extends ConsumerState<AddProductScreen> {
  ...
  void _processScannedBarcode(String code) async {
    ...
    if (existingProduct != null) {
      ...
      if (!isFresh) {
        ...
        final offProduct = await offService.fetchProductByBarcode(
          searchBarcode,
          dataSaver: ref.read(settingsProvider).enableDataSaver,
        );
        ...
        if (offProduct != null && mounted) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                'Prodotto trovato su Open Food Facts: ${offProduct.name}',
              ),
  ...
  Future<String?> _saveImageLocally(String barcode) async {
    ...
    try {
      ...
      final fileName =
          '${barcode}_${DateTime.now().millisecondsSinceEpoch}.jpg';
  ...
  void _saveProduct() async {
    if (_formKey.currentState!.validate()) {
      ...
      if (finalBarcode.isEmpty) {
        finalBarcode =
            'MANUAL_${const Uuid().v4().substring(0, 8).toUpperCase()}';
      }
      ...
      final newProduct = Product(
        ...
        imageUrl:
            localImagePath ??
            _remoteImageUrl ??
            (File(_barcodeController.text).existsSync()
      ...
      if (_selectedStoreId != null && priceValue != null && isStoreValid) {
        ...
        String? cleanPromoType = _promoType == AppStrings.promoNone
            ? null
            : _promoType;
        if (cleanPromoType == AppStrings.promoDiscountPercent &&
        ...
        final existingHistory = await ref
            .read(priceHistoryProvider)
            .getHistoryForProduct(finalBarcode);
        final storeHistory = existingHistory
            .where((h) => h.storeId == _selectedStoreId)
            .firstOrNull;

        if (storeHistory != null &&
            storeHistory.promoType != null &&
            storeHistory.promoType != cleanPromoType) {
          if (mounted) {
            final bool? confirmed = await showDialog<bool>(
              ...
              builder: (ctx) => AlertDialog(
                ...
                content: Text(
                  'In questo punto vendita esiste già una promozione di tipo "${storeHistory.promoType}". Vuoi sostituirla con "$cleanPromoType"?',
                ),
                actions: [
                  TextButton(
        ...
        if (cleanPromoType != null) {
          ...
          final existingPromo = promos
              .where(
                (p) =>
                    p.storeId == _selectedStoreId && p.name == cleanPromoType,
              )
              .firstOrNull;
          ...
          if (existingPromo == null) {
            ...
            final newPromo = Promotion(
              ...
              name: cleanPromoType,
              storeId: _selectedStoreId!,
              discountPercentage: _promoType == AppStrings.promoDiscountPercent
          ...
          await SpesAppDatabaseHelper.instance.linkProductToPromotion(
            promoIdToLink,
            finalBarcode,
          );
        }
                  ...
                  children: [
                    ...
                    TextFormField(
                      ...
                      validator: (value) => value == null || value.isEmpty
                          ? AppStrings.productNameValidator
                          : null,
                    ),
                    const SizedBox(height: 16),
                    if (!widget.isFastMode) ...[
                    ...
                    children: [
                      if (_selectedStoreId == null) ...[
                        DropdownButtonFormField<String>(
                          ...
                          value: stores.any((s) => s.id == _selectedStoreId)
                              ? _selectedStoreId
                              : null,
                          items: stores
                              .map(
                          ...
                          validator: (value) => value == null
                              ? AppStrings.validationStoreRequired
                              : null,
                        ),
                        const SizedBox(height: 16),
                      ],
                          ...
                          if (value.text.isNotEmpty) {
                            return Padding(
                              ...
                              child: Align(
                                ...
                                child: Text(
                                  'Prezzo Unitario: ${value.text} €/$_weightUnit',
                                  style: const TextStyle(
                                    color: Colors.indigo,
                                    fontSize: 13,
